...
@interface SMGHorizontalPickerView () {
...
@implementation SMGHorizontalPickerView : UIView
...
- (void)initSetup {
  ...
  self.indicatorPosition = SMGHorizontalPickerIndicatorBottom;
  ...
  for (UIView *view in [_scrollView subviews]) {
    ...
    if (!CGRectIntersectsRect(scaledViewFrame, visibleBounds)) {
      ...
      if ([view respondsToSelector:setSelectedSelector]) {
        ...
        // casting to SMGHorizontalPickerLabel so we can call this without all the NSInvocation jazz
        [(SMGHorizontalPickerLabel *)view setSelectedElement:isSelected];
      }
    }
  }
...
- (void)setIndicatorPosition:(SMGHorizontalPickerIndicatorPosition)position {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  if (self.scrollingBasedOnUserInteraction) {
    ...
    // notify delegate of contentOffset
    SEL delegateCall = @selector(horizontalPickerView:contentOffset:);
    if (self.delegate && [self.delegate respondsToSelector:delegateCall]) {
      [self.delegate horizontalPickerView:self contentOffset:scrollView.contentOffset];
    }
...
- (void)addScrollView {
  if (_scrollView == nil) {
    ...
    UILongPressGestureRecognizer *longPressRecognizer = [[UILongPressGestureRecognizer alloc]
                                               initWithTarget:self action:@selector(scrollViewLongPressed:)];
...
- (void)drawPositionIndicator {
  ...
  switch (self.indicatorPosition) {
    case SMGHorizontalPickerIndicatorTop: {
      y = 0.0f;
      break;
    }
    case SMGHorizontalPickerIndicatorBottom: {
...
- (SMGHorizontalPickerLabel *)labelForForElementAtIndex:(NSInteger)index withTitle:(NSString *)title {
...
#pragma mark - Long Press Gesture Recognizer Handler Method
// use the long press gesture recognizer to do something unique
- (void)scrollViewLongPressed:(UILongPressGestureRecognizer *)recognizer  {
  if (recognizer.state == UIGestureRecognizerStateRecognized) {
    CGPoint pressLocation    = [recognizer locationInView:_scrollView];
    NSInteger elementIndex = [self elementContainingPoint:pressLocation];
    if (elementIndex != -1) { // point not in element
      // notify delegate
      SEL delegateCall = @selector(horizontalPickerView:longPressDetectedAtIndex:);
      if (self.delegate && [self.delegate respondsToSelector:delegateCall]) {
        [self.delegate horizontalPickerView:self longPressDetectedAtIndex:elementIndex];
      }
...
@implementation SMGHorizontalPickerLabel : UILabel
